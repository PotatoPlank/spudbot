...
class Numeric
{
     ...
     * @param string $input
     * @return string
     * Credit to thefish on Github
     * @link https://github.com/thefish/words-to-number-converter
     */
    public static function wordsToNumber(string $input): string
    {
        static $delimiters = " \-,.!?:;\\/&\(\)\[\]";
        static $tokens = [
            'zero' => ['val' => '0', 'power' => 1],
        ...
        $mutate = function ($parts) use (&$mutate, $powers) {
            ...
            foreach ($parts as $idx => $arr) {
                ...
                if (!$stack->isEmpty()) {
                    ...
                    )) { //пропускаем спец числительные
                        return $stack->top() . ($parts[$idx - $stack->count()]['suffix'] ?? '') . " " . $mutate(
                                array_slice($parts, $idx)
                            );
                    }
        ...
        $prepared = preg_split('/([' . $delimiters . '])/', $input, -1, PREG_SPLIT_DELIM_CAPTURE);

        foreach ($prepared as $idx => $word) {
            if (is_array($word)) {
                continue;
            }
            $maybeNumPart = strtolower(trim($word));
