<?php
/*
 * This file is a part of the SpudBot Framework.
 * Copyright (c) 2024-2025. PotatoPlank <potatoplank@protonmail.com>
 * The file is subject to the GNU GPLv3 license that is bundled with this source code in LICENSE.md.
 */

namespace Spudbot\Events\Ready;

use Spudbot\Bot\Events;
use Spudbot\Events\AbstractEventSubscriber;

class IntervalLoop extends AbstractEventSubscriber
{

    public function getEventName(): string
    {
        return Events::READY->value;
    }

    public function update(): void
    {
        $this->spud->discord->getLoop()->addPeriodicTimer($this->getSecondsInMinutes(0.1), function () {
            $this->spud->discord->emit(Events::EVERY_TEN_SECONDS->value);
        });
        $this->spud->discord->getLoop()->addPeriodicTimer($this->getSecondsInMinutes(0.5), function () {
            $this->spud->discord->emit(Events::EVERY_HALF_MINUTE->value);
        });
        $this->spud->discord->getLoop()->addPeriodicTimer($this->getSecondsInMinutes(1), function () {
            $this->spud->discord->emit(Events::EVERY_MINUTE->value);
        });
        $this->spud->discord->getLoop()->addPeriodicTimer($this->getSecondsInMinutes(10), function () {
            $this->spud->discord->emit(Events::EVERY_TEN_MINUTES->value);
        });
    }

    protected function getSecondsInMinutes(int|float $minutes): int
    {
        return $minutes * 60;
    }
}
