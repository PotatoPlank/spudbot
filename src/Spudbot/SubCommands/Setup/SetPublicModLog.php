...
class SetPublicModLog extends AbstractSubCommandSubscriber
{
    #[Inject]
    ...
    {
        return 'public_log_channel';
    }

    public function update(?Interaction $interaction = null): void
    {
        if (!$interaction) {
            throw new BadMethodCallException('Setting a Public Log Channel requires an interaction.');
        }
            ...
            ->done(function (Channel $channelPart) use ($interaction) {
                ...
                if ($isThread) {
                    $interaction->guild->channels->fetch($channelPart->parent_id)
                        ->done(function (Channel $parentPart) use ($channelPart, $guild) {
                ...
                $this->spud->interact()
                    ->setTitle('Set Public Mod Log')
                    ->setDescription(
                        "Set the public mod log."
    ...
    {
        $guild->setChannelPublicLogId($channelId);
        $guild->setChannelThreadPublicLogId($threadId);
