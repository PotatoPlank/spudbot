...
class SetIntroChannel extends AbstractSubCommandSubscriber
{
    #[Inject]
    ...
    {
        return 'intro_channel';
    }

    public function update(?Interaction $interaction = null): void
    {
        if (!$interaction) {
            throw new BadMethodCallException('Setting an Intro Channel requires an interaction.');
        }
        ...
        $interaction->guild->channels->fetch($channelId)
            ->done(function (Channel $channelPart) use ($interaction) {
                ...
                $this->spud->interact()
                    ->setTitle('Set Introduction Channel')
                    ->setDescription(
