...
class SetMarketplaceChannel extends AbstractSubCommandSubscriber
{
    #[Inject]
    ...
    {
        if (!$interaction) {
            throw new BadMethodCallException('Setting a Marketplace Channel requires an interaction.');
        }
        ...
        $interaction->guild->channels->fetch($channelId)
            ->done(function (Channel $channelPart) use ($interaction) {
                ...
                $this->spud->interact()
                    ->setTitle('Set Marketplace Channel')
                    ->setDescription(
                        "Set the marketplace channel."
